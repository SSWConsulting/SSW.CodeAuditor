<script>
  ...
  \`\`\` bash
  $ docker run sswconsulting/codeauditor --rm --cap-add=SYS_ADMIN --token ${token} --url <URL> 
  \`\`\`

  If you don't you your scan to be uploaded publicly, following command will upload your scan to your private profile:
  \`\`\` bash
  $ docker run sswconsulting/codeauditor --rm --cap-add=SYS_ADMIN --token ${token} --url <URL> --private
  \`\`\`
