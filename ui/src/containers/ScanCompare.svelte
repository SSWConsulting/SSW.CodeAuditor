...
<div class="container mx-auto mt-8">
  ...
  <div class="grid grid-cols-2 gap-x-6 text-center mt-8">
    <div class="text-lg text-gray-600 mb-2">Select Past Scan to compare:</div>
    <div class="text-lg text-gray-600 mb-2">Latest Scan at</div>
    <div>
      <select
      ...
      >
        {#each allScans.slice(1) as scan}
          <option value={scan}>
            {format(new Date(scan.buildDate), "h:mm bbb, dd LLL y")}
          </option>
        {/each}
      </select>
    </div>
    <div class="font-sans font-bold text-lg text-gray-900">
      {allScans.length > 0
        ? format(new Date(allScans[0].buildDate), "h:mm bbb, dd LLL y")
        : ""}
    </div>
    <div>
      {#if selectedScan}
